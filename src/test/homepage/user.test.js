...
describe('executing test scenario on ePAD', () => {
  ...
  beforeAll(() => {
    // create project for user test
    return new Promise(async resolve => {
      await login.login();
      await navbar.selectTabById('management');
      await navbar.selectMNGSubject('Projects');
      await driver.sleep(500);
      await projects.createSingleProject(
        'usersTestProject',
        'usersTestProject',
        'usersTestProject',
        null,
      ...
      await projects.clickByCss('.mng-header__close');
      resolve();
    });
  });

  afterAll(() => {
    //delete project and quit
    return new Promise(async done => {
      await projects.clickByCss('.mng-header__close');
      await navbar.selectTabById('management');
      await navbar.selectMNGSubject('Projects');
      await driver.sleep(500);
      await projects.singleDelete('usersTestProject');
      await driver.quit();
      done();
    });
  });
  ...
  test('it adds user 2 to a project', async () => {
    ...
    expect(project).toMatch(/default project1/);
  });
