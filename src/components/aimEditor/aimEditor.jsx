...
class AimEditor extends Component {
  ...
  componentDidUpdate(prevProps) {
    if (
      this.props.aimId !== undefined &&
      prevProps.aimId !== undefined &&
      prevProps.aimId.aimId !== this.props?.aimId?.aimId
    )
  ...
  checkMarkupsForTemplate = () => {
    ...
    if (templateType === 'Study' || templateType === 'Series') {
      ...
      if (!toolState || toolState === 'undefined') return;
  ...
  createAim = async (answers) => {
    ...
    if (hasSegmentation) {
      ...
      if (isStudyAim) {
        //Study annotation
        const { openSeries, activePort } = this.props;
        const { data: study } = await getSingleStudy(
          openSeries[activePort].studyUID
        );
        ...
        // Image Annotation, TODO:Might be series annotation too
        const { activePort } = this.props;
  ...
  createAimSegmentation = async (answers) => {
    ...
    const { segBlob, segStats, imageIdx, image } =
  ...
  saveAim = (aim, segmentationBlob, segId) => {
    ...
    const comment =
      aimSaved.ImageAnnotationCollection.imageAnnotations.ImageAnnotation[0]
        .comment.value;
    const isStudyAim =
      aim.getType() === enumAimType.studyAnnotation ? true : false;
    const aimRefs = {
      aimID,
      patientID,
      projectID,
  ...
  getAxisOfBidirectional = (bidirectional) => {
    ...
    rowPixelSpacing = rowPixelSpacing
      ? rowPixelSpacing
      : displayedArea
      ? displayedArea.rowPixelSpacing
      : 1;
    columnPixelSpacing = columnPixelSpacing
      ? columnPixelSpacing
      : displayedArea
      ? displayedArea.columnPixelSpacing
      : 1;
  ...
  createSegmentation3D = async (labelmapIndex, aimName) => {
    ...
    for (let i = 0; i < images.length; i++) {
      metadataArray.push(
        cornerstoneWADOImageLoader.wadors.metaDataManager.get(images[i].imageId)
      );
    }
    // const dataset = dcmjs.data.DicomMetaDictionary.naturalizeDataset(instance);
    const segBlob =
      dcmjs.adapters.Cornerstone.Segmentation.generateSegmentation(
        metadataArray,
        labelmap3D,
        { rleEncode: false }
      );
    ...
    const data = this.createImageDataFromMetadata(
      images[firstSegImageIndex].imageId
    );
  ...
  uploadCompleted = (aimRefs) => {
    ...
    if (isStudyAim) {
      ...
      if (encrypted === true && mode === 'teaching')
        this.props.dispatch(clearAimId());
      openSeries.forEach(({ seriesUID, studyUID }) => {
        if (
          openSeries[activePort].studyUID === studyUID &&
          openSeries[activePort].seriesUID !== seriesUID
        )
          this.props.dispatch(
            getSingleSerie(
              { patientID, projectID, seriesUID, studyUID },
              null,
              null,
              existingData
            )
